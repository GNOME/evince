...
view_update_range_and_current_page (EvView *view)
{
	...
	if (view->continuous) {
		...
		for (i = 0; i < n_pages; i++) {
			...
			ev_view_get_page_extents_for_border (view, i, &page_area, border);
		       ...
		       gint    *y_offset)
{
	...
	for (i = view->start_page; i >= 0 && i <= view->end_page; i++) {
		...
		if (! ev_view_get_page_extents_for_border (view, i, &page_area, border))
			continue;
			...
			GList  *new_list)
{
	...
	while (new_list_ptr || old_list_ptr) {
		...
		if (region) {
			...
			ev_view_get_page_extents_for_border (view, cur_page, &page_area, border);
